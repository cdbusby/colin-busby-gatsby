...
const ArticleTemplate = ({ data }) => {
  var metaDescription = data.article.field_teaser.processed.replace(
    /(<([^>]+)>)/gi,
    ""
  )
    ...
    tags = (
      <div className="w-full sm:w-1/2 my-4 px-4">
        ...
        {data.article.relationships.field_tags.map((tag) => (
          <Link
            to={`/tags/${tag.name}`}
            className="mr-2 py-1 px-2 rounded-md bg-indigo-600 text-white hover:text-white text-sm no-underline"
          >
            {tag.name}
          </Link>
        ))}
